                ...
                if (toPrint)
                {
                    ...
                    while (current->getDataKey(i).key_value == keyTofind)
                    {
                        for (int j = 0; j < current->getDataKey(i).add.size(); j++)
                        {
                            ...
                            if (count > 0)
                            {
                                cout << "| " << (*(Record *)current->getDataKey(i).add[j]).tconst << "      "
                                     << (*(Record *)current->getDataKey(i).add[j]).averageRating << "          "
                                     << (*(Record *)current->getDataKey(i).add[j]).numVotes << endl;
                                count--;
                            }
                            totalRating += (*(Record *)current->getDataKey(i).add[j]).averageRating;
